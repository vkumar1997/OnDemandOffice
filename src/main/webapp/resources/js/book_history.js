...
function populate(result)
{
	if(result.length==0)
	{
		...
		message.innerHTML="No Bookings Made";
	...
	else
	{
		...
		for(i=0; i<result.length; i++)
		{
			...
			var owner_name=document.createElement("div");
			owner_name.className +='entry';
			var key_name=document.createElement("label");
			key_name.className+='key';
			key_name.innerHTML="Owner Name:<br>";
			var value_name=document.createElement("label");
			value_name.className+='value';
			value_name.innerHTML=result[i].owner_name;
			owner_name.appendChild(key_name);
			owner_name.appendChild(value_name);
			container.appendChild(owner_name);
			
			var owner_mail=document.createElement("div");
			owner_mail.className +='entry';
			...
			value_mail.innerHTML=result[i].owner_mail;
			owner_mail.appendChild(key_mail);
			owner_mail.appendChild(value_mail);
			container.appendChild(owner_mail);
			
			var owner_phone=document.createElement("div");
			owner_phone.className +='entry';
			...
			value_phone.innerHTML=result[i].owner_phone;
			owner_phone.appendChild(key_phone);
			owner_phone.appendChild(value_phone);
			container.appendChild(owner_phone);
			...
			var ref_id=document.createElement("div");
			ref_id.className +='entry';
			var key_ref=document.createElement("label");
			key_ref.className+='key';
			key_ref.innerHTML="Reference Id:<br>";
			var value_ref=document.createElement("label");
			value_ref.className+='value';
			value_ref.innerHTML=result[i].ref_id;
			ref_id.appendChild(key_ref);
			ref_id.appendChild(value_ref);
			container.appendChild(ref_id);
			...
			if(result[i].status!="Active")
			{
				...
				key_earn.innerHTML="Total Cost:<br>";
